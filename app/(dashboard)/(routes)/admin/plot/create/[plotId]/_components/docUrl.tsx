...
export const DocUrlForm = ({ saleId, initialData }: DocUrlFormProps) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      await axios.patch(`/api/plotSale/${saleId}`, {
        customerIdProofUrl: values.docUrl,
      });
    ...
    <div className="mt-6 border  rounded-md p-4 ">
      ...
      {!isEditing && (
        <p className="text-sm mt-2">
          {(
            <Link
              href={initialData.customerIdProofUrl}
              color="warning"
              underline="always"
            >
      ...
      {isEditing && (
        <form onSubmit={handleSubmit(onSubmit)} className="my-3">
          <Input
            ...
            size="sm"
            variant="flat"
            placeholder="Please paste the url link here.."
            defaultValue={initialData.customerIdProofUrl}
