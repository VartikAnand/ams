...
export const SalaryTableAction = ({ user }: FarmerTableActionProps) => {
  ...
  const handleItemClick = async (key: string) => {
    ...
    try {
      if (key === "edit") {
        await router.push(`/admin/salary/create/${user?.empId}`);
        const promise = () =>
      ...
      //Add payment Create
        ...
        try {
          const response = await axios.post(
            `/api/salary/${user.empId}/employeePayment`,
            {
              empId: user.empId,
      ...
      // vIEW rECORD
        ...
        router.push(`/admin/salary/view/${user.empId}`);
      } else {
