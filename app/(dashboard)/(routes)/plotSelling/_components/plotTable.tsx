...
import { columns, statusOptions } from "./plotTableData.ts";
...
export const PlotTable = ({ initialData, userData }: PlotTableProps) => {
  ...
  const renderCell = useCallback((user: UserData, columnKey: React.Key) => {
    ...
    switch (columnKey) {
      ...
      case "salesInvoiceNumber":
        return <p>{`${user.salesInvoiceNumber}`}</p>;

      //userName
      case "plotNumber":
        return <p>{user?.plotNumber}</p>;

      // firstName
      case "plotPrice":
        return <p>₹{Number(user?.plotPrice).toLocaleString()}</p>;

      case "plotArea":
        return <p>{Number(user?.plotArea).toLocaleString()} m²</p>;
      //lastName
      case "customerName":
      ...
      // phoneNumber
      case "phoneNumber":
        return <p>{user?.custommerPhoneNumber}</p>;
      // email
      case "customerAddress":
        return (
          <p>
            {`  ${user?.custommerAddress},
  ${user?.customerCity} ${user?.customerState},
  ${user?.customerPinCode}`}
          </p>
        );
      case "customerIdProofUrl":
        return (
          <p>
            <Link href={user?.customerIdProofUrl} color="primary">
              {`Docs Url : ${user?.customerName} `}
            </Link>
          </p>
        );
      ...
      case "saleDate":
        return (
          <p>
            {user?.saleDate?.toLocaleString("en-US", {
              day: "numeric",
              month: "short",
  ...
  return (
    <div className="lg:mx-5 mx-2 ">
      <Table
        onRowAction={(key) => router.push(`/plotSelling/view/${key}`)}
        isCompact
      ...
      >
        ...
        <TableBody
          color="primary"
          emptyContent={"No Plot Sale Data Found "}
          items={sortedItems}
        >
          {(item) => (
