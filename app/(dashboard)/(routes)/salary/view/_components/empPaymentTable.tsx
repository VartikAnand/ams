...
import { columns, statusOptions } from "./empData.ts";
...
export const EmpPaymentTable = ({ farmerId, initialData, farmerData }) => {
  ...
  const renderCell = useCallback((user: UserData, columnKey: React.Key) => {
    ...
    switch (columnKey) {
      //paid amount
      case "salary":
        return <p>&#8377; {user?.salary?.toLocaleString("en-IN") || "0"}</p>;
      case "netSalary":
        return <p>&#8377; {user?.netSalary?.toLocaleString("en-IN") || "0"}</p>;

      // bonus
      case "bonus":
        return (
          <p className="text-primary">
            {user?.bonus ? `₹ ${user.bonus.toLocaleString("en-IN")}` : "Nil"}
          </p>
        );
      // bONUS dATE
      case "bonusDate":
      ...
      // paymentGivenBy
      case "paymentGivenBy":
        return <p>{user?.paymentGivenBy}</p>;
      //Payment Type
      case "isPaymentAdded":
        return (
          <Chip
